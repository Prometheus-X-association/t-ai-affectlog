...
def validate_csv(
    ...
    schema: str = typer.Option(
        "maskott_csv_v1", "--schema", "-s", help="Schema name to validate against"
    ),
    ...
    if result.get("valid"):
        ...
        if result.get("extra_columns"):
            console.print(
                f"[yellow]Extra columns (not in schema): {result['extra_columns']}[/yellow]"
            )
    else:
...
def infer_json_template(
    ...
    output: Path | None = typer.Option(
        None, "--output", "-o", help="Output path for inferred template"
    ),
) -> None:
...
def convert_csv(
    ...
    template: Path | None = typer.Option(None, "--template", "-t", help="Becomino JSON template"),
    output: Path = typer.Option(..., "--output", "-o", help="Output JSONL path"),
    _format: str = typer.Option("jsonl", "--format", "-f"),
    ...
    vm = VerbMapper(rec.xapi.verb_mapping)  # type: ignore[arg-type]
...
def compliance_export(
    ...
    _format: str = typer.Option("jsonld", "--format", "-f", help="Export format: jsonld"),
