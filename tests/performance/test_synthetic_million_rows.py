...
def test_generate_1m_rows(tmp_path):
    ...
    assert out.exists()
    line_count = sum(1 for _ in out.open()) - 1  # minus header
...
def test_stream_convert_1m_rows(tmp_path):
    ...
    pseudo = Pseudonymizer(
        secret="bench-secret", hash_fields=["EntityId", "_id", "EntityUaiCode", "ActivitySessionId"]
    )
